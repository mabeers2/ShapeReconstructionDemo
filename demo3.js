...
import experiment_data from '/demo_dataset3.json' assert {type: 'json'};
import { LineMaterial } from 'three/addons/lines/LineMaterial.js';
import { LineSegments2 } from 'three/addons/lines/LineSegments2.js';
import { LineSegmentsGeometry } from 'three/addons/lines/LineSegmentsGeometry.js';

console.log('does this do anything', LineMaterial)
...
for (var i=0, len=experiment_data.xyz.length; i<len; i++) { 
  ...
  sa.push(Math.PI / 180 * experiment_data.vsa[i])
}
var sa_init = [...sa]
...
function init(k, cam_dist){ 
  // Make canvas
  scene = new THREE.Scene() 

  // Update Title
  document.getElementById("info").innerHTML = "Trial " + (k +1).toString() + " / " + (experiment_data.xyz.length).toString();
  ...
  const preRight = roty(-1 * 180 / Math.PI * 0)
  ...
  // left_cube.name = "left"
  // const geometry_test = new LineSegmentsGeometry();
  // geometry_test.setPositions( xyz_left._data );
  // geometry_test.setColors( xyz_left._data );
  // console.log(xyz_left._data);
  // var edge_material3 = new LineMaterial( { color: 0xF54927 , linewidth:.01} );
  // var wire_left = new LineSegments2( geometry_test, edge_material3 );
  // wire_left.position.x = 0;
  // wire_left.position.z = -10;
  // scene.add( wire_left );
  // wire_left.position.z = 1;
  // console.log(wire_left);
  // console.log("XYZ left = ", xyz_left._data)
  ...
  shrink_level = 1//.5 + .2 * Math.random()
  ...
  right_cube.rotation.y = 0;//Math.PI/2
  ...
  // var edge_material2 = new LineMaterial( { linewidth: 5, vertexColors: false });
  // edge_material2.color.set(1,0,0);
  // var wire_left = new LineSegments2( geometry_left, edge_material2 );
  // wire_left.position.x = left_x;
  // scene.add( wire_left );
  // var wire_right = new LineSegments2( geometry_right, edge_material2 );
  const geometry_test = new LineSegmentsGeometry();
  geometry_test.setPositions( xyz_right._data );
  // geometry_test.setColors( xyz_left._data );
  // console.log(xyz_left._data);
  var edge_material3 = new LineMaterial( { color: 0xF54927 , linewidth:.005} );
  var wire_right = new LineSegments2( geometry_test, edge_material3 );

  wire_right.position.x = right_x;
  // wire_right.rotation.y = 0;//Math.PI/2
...
function animate() {
  ...
  object.rotation.y = .3 * Math.sin(rot_speed * 2);
  wireframe.rotation.y = .3 * Math.sin(rot_speed * 2);
  // object.rotation.x = rot_speed * 2;
  // wireframe.rotation.x = rot_speed * 2;
...
function onDownload(){
  ...
  for (var i=0, len=experiment_data.xyz.length; i<len; i++){
    ...
    isa = sa_init[i] * 180 / Math.PI
    rsa = (180/Math.PI * sa[i]).toString()
    d = 4
  ...
  a.download = 'demo_' + localStorage.getItem("initials") + '.csv';
